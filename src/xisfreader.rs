...
pub fn xisf_type_size(xisf_type: &str) -> u8 {
    match xisf_type {
        ...
        _ => unreachable!("unsupported XISF type found: {}", xisf_type),
    }
}
...
fn xisf_parse_xml(
    ...
    let mut reader = Reader::from_str(&xisf_header.header);
    let mut buf = Vec::new();
                        ...
                        for attr in e.attributes() {
                            ...
                            match attr.key {
                                b"geometry" => {
                                    ...
                                    xisf_header.geometry =
                                        str::from_utf8(&attr.value).unwrap().to_owned();

                                    if xisf_header.geometry.contains(':') {
                                        let mut iter = xisf_header.geometry.split(':');

                                        let size_x = iter.next().unwrap().parse::<u64>().unwrap();
                                        let size_y = iter.next().unwrap().parse::<u64>().unwrap();
                                        let num_channels =
                                            iter.next().unwrap().parse::<u32>().unwrap();

                                        xisf_header.geometry_channel_size = size_x * size_y;
                                        xisf_header.geometry_channels = num_channels;
                                    }
                                }
                                b"sampleFormat" => {
                                    // Parse image format
                                ...
                                b"compression" => {
                                    ...
                                    xisf_header.compression =
                                        str::from_utf8(&attr.value).unwrap().to_owned();
                                    let mut iter = xisf_header.compression.split(':');

                                    xisf_header.compression_codec = iter.next().unwrap().to_owned();
...
/// Print header data
fn xisf_print_header_info(xisf_header: &XISFHeader) {
    // Print header values
    info!("Header signature: {}", xisf_header.signature);

    info!("Length: {}", xisf_header.length);
    info!("Reserved: {}", xisf_header.reserved);
    info!("Header: {}", xisf_header.header);

    info!("Geometry: {}", xisf_header.geometry);
    info!("Geometry sizes: {:?}", xisf_header.geometry_sizes);
    info!("Geometry channels: {}", xisf_header.geometry_channels);
    info!(
    ...
    info!("Location start: {}", xisf_header.location_start);
    info!("Location length: {}", xisf_header.location_length);
    info!(
...
pub fn xisf_read_file(
    ...
    xisf_header.header = buffer_header_header;
    ...
    // Output parsed data
    xisf_print_header_info(&xisf_header);
