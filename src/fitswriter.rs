...
fn fits_write_header_u64(fits: &mut File, header: &String, value: u64, bytes: &mut u64) -> io::Result<()> {
...
fn fits_write_header_i64(fits: &mut File, header: &String, value: i64, bytes: &mut u64) -> io::Result<()> {
    let string = format!("{:8} = {:>19} /{:48}", header, value, "");
    println!("FITS header: \"{}\"", string);
    let header_bytes = string.as_bytes();
    fits.write(header_bytes)?;
...
fn fits_write_header_string(fits: &mut File, header: &String, value: &String, bytes: &mut u64) -> io::Result<()> {
    let string = format!("{:8} = {:>19} /{:48}", header, value, "");
    println!("FITS header: \"{}\"", string);
    let header_bytes = string.as_bytes();
    fits.write(header_bytes)?;
